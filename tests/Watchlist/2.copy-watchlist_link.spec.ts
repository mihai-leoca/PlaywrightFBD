import { Page, test, expect, errors } from "@playwright/test";
...
test.describe.serial(`Create a new watchlist`, async () => {
  ...
  test("Copy a watchlist link", async ({}) => {
    await watchlistPO.copyWatchlistLink();
    await expect(page.getByRole('heading', { name: 'Link copied! This link is single-use for privacy reasons and expires in 14 days' })).toBeVisible();
    await expect(page.getByRole('heading', errorMessagesPO.defaultErrorMsg)).not.toBeVisible();
  });
